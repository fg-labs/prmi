...
pub fn lookup_core<A: Layer + ?Sized, B: Layer + ?Sized>(
    ...
    if l2_idx >= l2.len() {
        // Corrupt sidecar: `bit_shift` disagrees with `l2.len()`, so the routing
        // index falls outside the L2 array. The Fulcrum trainer cannot produce
        // this, but a tampered sidecar could. There is no L2 entry to read, so
        // return a clamped position with the widest possible error; the caller's
        // bounded search then covers the whole SA and stays correct.
    ...
    if (err >> 63) != 0 {
        ...
        if partial_num == 0 {
            // Corrupt sidecar: a fallback L2 entry encoded partial_num=0.
            // The Fulcrum trainer cannot produce this, but a tampered sidecar
            // could. Fall back to using the L2 entry's own prediction
            // (already in fpred) clamped to the SA range. The resulting err
            // will be wide; the caller's bounded search will still be correct.
            let pos = clamp_to_int(fpred, 0.0, sa_num.saturating_sub(1) as f64) as u64;
            return (pos, err);
        }
        if partial_start >= l1.len() || partial_start + (partial_num - 1) >= l1.len() {
            // Corrupt sidecar: the encoded L1 fallback range exceeds the L1
            // array. Fall back to the L2 entry's own prediction (already in
            // `fpred`) clamped to the SA range, exactly as the `partial_num == 0`
            // branch above does.
            let pos = clamp_to_int(fpred, 0.0, sa_num.saturating_sub(1) as f64) as u64;
            return (pos, err);
        }

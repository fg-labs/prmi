...
fn open_missing_file_returns_negative_and_error() {
    ...
    let mut handle = ptr::null_mut();
    let rc = unsafe { prmi_open(cprefix.as_ptr(), &mut handle) };
    assert_eq!(rc, -3, "missing sidecar should return -3, got {rc}");
    assert!(handle.is_null());
...
#[test]
fn open_null_arg_clears_stale_handle() {
    // A caller reusing a handle variable must not be left with a stale pointer
    // when a reopen fails on a null argument: `*out_handle` is cleared first.
    let mut handle = 0xDEAD_BEEFusize as *mut _;
    let rc = unsafe { prmi_open(ptr::null(), &mut handle) };
    assert_eq!(rc, -1);
    assert!(
        handle.is_null(),
        "stale handle must be cleared to NULL on the null-argument error path"
    );
}

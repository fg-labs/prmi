...
fn ffi_matches_rust_full_32mer() {
    ...
    let mut out: u64 = 0;
    let rc = unsafe { prmi_tokenize_32mer(bases.as_ptr(), bases.len(), 32, &mut out) };
...
fn ffi_matches_rust_short_query() {
    ...
    let mut out: u64 = 0;
    let rc = unsafe { prmi_tokenize_32mer(bases.as_ptr(), bases.len(), 4, &mut out) };
    assert_eq!(rc, 0);
...
fn ffi_zero_len_gives_all_t_pad() {
    ...
    let mut out: u64 = 0;
    let rc = unsafe { prmi_tokenize_32mer(bases.as_ptr(), bases.len(), 0, &mut out) };
    assert_eq!(rc, 0);
    assert_eq!(out, expected);
}
...
fn ffi_len_over_32_is_clamped() {
    ...
    let mut out: u64 = 0;
    // Pass len=64 — should clamp to 32 internally (also clamped by bases_len=64).
    let rc = unsafe { prmi_tokenize_32mer(bases.as_ptr(), bases.len(), 64, &mut out) };
    assert_eq!(rc, 0);
    assert_eq!(out, expected);
}

/// bases_len smaller than len clamps the effective length.
#[test]
fn ffi_bases_len_clamps_len() {
    // 32 bases available, but we tell the FFI only 4 are valid.
    let bases: Vec<u8> = (0u8..32).map(|i| i % 4).collect();
    let expected = tokenize_32mer(&bases[..4], 4);

    let mut out: u64 = 0;
    // bases_len=4 should clamp n to min(32, 4, 32) = 4.
    let rc = unsafe { prmi_tokenize_32mer(bases.as_ptr(), 4, 32, &mut out) };
    assert_eq!(rc, 0);
    assert_eq!(out, expected);
}
...
fn ffi_null_bases_returns_error() {
    let mut out: u64 = 0;
    let rc = unsafe { prmi_tokenize_32mer(ptr::null(), 4, 4, &mut out) };
    assert_eq!(rc, -1);
}
...
fn ffi_null_out_key_returns_error() {
    ...
    let rc = unsafe { prmi_tokenize_32mer(bases.as_ptr(), bases.len(), 4, ptr::null_mut()) };
    assert_eq!(rc, -1);
}
...
fn ffi_negative_len_returns_error() {
    let bases: Vec<u8> = vec![0u8; 4];
    let mut out: u64 = 0;
    let rc = unsafe { prmi_tokenize_32mer(bases.as_ptr(), bases.len(), -1, &mut out) };
    assert_eq!(rc, -1);
}

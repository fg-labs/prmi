...
fn lookup_returns_zero_on_known_key() {
    ...
    assert_eq!(rc, 0);
    // The FFI outputs must match the in-tree Rust reference exactly, so a
    // regression that stops writing either out-pointer is caught.
    let (expected_pos, expected_err) = prmi::index::LearnedIndex::open(&prefix).unwrap().lookup(0);
    assert_eq!(pos, expected_pos, "pos must match LearnedIndex::lookup");
    assert_eq!(err, expected_err, "err must match LearnedIndex::lookup");
